...
async def analytic(payload:InputFormat):
    try:
        ...
        logger.error(f"Internal services error. Can not process Applicant id [{payload["applicant_id"]}]")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal services error")